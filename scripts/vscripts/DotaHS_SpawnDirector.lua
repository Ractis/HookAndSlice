...
-- Pre-Initialize
--------------------------------------------------------------------------------
function SpawnDirector:PreInitialize()
	...
	-- Register Think
	DotaHS_CreateThink( "SpawnDirector:OnUpdate", function ()
		self:OnUpdate()
		return SpawnDirector.DeltaTime
	end )
...
function SpawnDirector:Initialize( kv )
	...
	self._flHordeIntervalReduction = 0.0
...
--------------------------------------------------------------------------------
function SpawnDirector:SetHordeIntervalReduction( reduction )
	local oldReduction = self._flHordeIntervalReduction
	self._flHordeIntervalReduction = reduction

	-- TODO: Immediately apply to current horde timer?

	self:_Log( "Horde interval reduction has been changed to " .. reduction .. " from " .. oldReduction )
end

--------------------------------------------------------------------------------
function SpawnDirector:OnUpdate()
...
function SpawnDirector:_UpdateIntensity()
	...
	DotaHS_ForEachPlayer( function ( playerID, hero )
		...
		-- Update
		intensity:Update( SpawnDirector.DeltaTime )
...
function SpawnDirector:_UpdateHorde()
	...
	if self._hordeDue == nil then
		...
		duration = duration * ( 1.0 - self._flHordeIntervalReduction )
